...
- **Build banner** — the connector logs `RCC: v<version> loaded` (always visible, not verbose-gated) on init, so you can confirm the connector loaded and which version CloudCannon served. The version is stamped into the bundle at `npm run build`.
