...
- **Stale translation detection** — when source text changes after a translation was last acknowledged, the element gets an amber dashed border and the FAB shows a count badge. A floating panel lets editors resolve items individually or in bulk. Detection combines a build-time signal (`_base_original` vs `original`) with a live in-editor signal (current page source vs `original`), so an in-session source edit flags affected translations immediately, before any rebuild.
- **Untranslated indicator** — elements with no entry yet, or whose `value` still equals `original`, get a distinct teal dotted border and a teal count badge on the FAB, separate from amber stale. Untranslated takes visual precedence over stale (an untranslated entry has no translation to be out of date).
- **Build banner** — the connector logs `RCC: v<version> loaded (built <timestamp>)` (always visible, not verbose-gated) on init, so you can confirm which build CloudCannon actually served. The timestamp is stamped into the bundle at `npm run build`.
