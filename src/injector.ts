...
function updateStaleList(): void {
	...
	const body = submenu.querySelector<HTMLElement>("[data-rcc-stale-body]");
	if (body) body.style.display = isExpanded ? "block" : "none";

	const chevron = submenu.querySelector<HTMLElement>("[data-rcc-stale-chevron]");
	if (chevron) chevron.style.transform = isExpanded ? "rotate(90deg)" : "rotate(0deg)";

	const list = submenu.querySelector<HTMLElement>("[data-rcc-stale-items]");
	if (!list) return;
	list.innerHTML = "";
	...
	const resolveAllBtn = submenu.querySelector<HTMLElement>("[data-rcc-resolve-all]");
...
function injectSwitcher(locales: string[]): void {
	...
	for (const locale of locales) {
		...
		toggleHeader.addEventListener("click", () => {
			...
			const bodyEl = submenu.querySelector<HTMLElement>("[data-rcc-stale-body]");
