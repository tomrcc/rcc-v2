...
<!--
  Four deterministic states, keyed so the crafted rosey/locales/fr.json
  drives each one:
    ...
    stale:fresh        — no locale entry at all: the postbuild strips it back out
                         after write-locales seeds it → create-entry path
-->
...
<p data-rosey="fresh">This phrase has no locale entry at all.</p>
